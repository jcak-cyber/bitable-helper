...
import { Segmented, Spin, Button, Typography, Empty } from 'antd';
...
interface Props {
  /** 是否为当前激活 Tab；切回时强制刷新，避免切表后状态滞后 */
  active?: boolean;
}

export function WorkHourPanel({ active = true }: Props) {
  ...
  const {
    ...
    loading,
    refresh,
  } = useTableData(config?.linkFieldId);
  const selectedRecordId = useSelection();

  // 切回「工时管理」Tab 时强制按当前表重载（不依赖 refresh 引用，避免与 load 形成循环）
  useEffect(() => {
    if (active) refresh();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 仅在 Tab 激活时触发
  }, [active]);
  ...
  const handleSubmit = async (data: {
    ...
    if (result.failed > 0) {
      toast('error', `成功 ${result.success} 条工时，失败 ${result.failed} 个任务`);
    } else {
      toast('success', `已创建 ${result.success} 条工时记录`);
    }
    refresh();
  };
  ...
  if (!isTaskTable) {
    return (
      <div className="work-hour-panel panel-empty">
        <Empty
          image={Empty.PRESENTED_IMAGE_SIMPLE}
          description={
            <div className="empty-copy">
              <div className="empty-title">尚未打开任务管理表</div>
              <div className="empty-desc">
                请切换到「{TASK_TABLE_NAME}」数据表后使用工时管理
              </div>
              {tableName ? <div className="empty-hint">当前表：{tableName}</div> : null}
              <Button type="link" size="small" onClick={() => refresh()} style={{ padding: 0 }}>
                已切换？点击刷新
              </Button>
            </div>
          }
  ...
  return (
    <div className="work-hour-panel">
      <section className="mode-block">
        <div className="panel-toolbar">
          <div className="panel-toolbar-main">
            <span className="panel-toolbar-label">填写模式</span>
            <Typography.Text className="panel-toolbar-hint">
              {mode === 'single' ? '跟随表格选中行' : '勾选多条后批量创建'}
            </Typography.Text>
          </div>
          <Button type="link" size="small" className="panel-toolbar-action" onClick={startConfig}>
            重新配置
          </Button>
        </div>
        ...
        {mode === 'single' ? (
          selectedRecordId ? (
            <div className="selected-task">
              <span className="selected-task-label">将为以下任务创建工时</span>
              <Typography.Text strong ellipsis className="selected-task-title">
                {selectedTitle || '(未命名任务)'}
              </Typography.Text>
            </div>
          ) : (
            <div className="inline-hint warn">请在表格中选中一个任务</div>
          )
        ) : (
          <div className={`inline-hint ${targetIds.length > 0 ? 'accent' : 'warn'}`}>
            已勾选 <strong>{targetIds.length}</strong> 个任务
          </div>
        )}
      ...
      <WorkLogForm
        ...
        canSync={Boolean(config.dateFieldId)}
